...
class Command(BaseCommand):
    ...
    def _import_media(self, records, stats):
        ...
        Uses filter().first() instead of update_or_create to safely
        handle duplicate filenames without triggering MultipleObjectsReturned.
        """
        from apps.catalog.models import Media
        ...
        for i, r in enumerate(records, 1):
            ...
            existing = Media.objects.filter(filename=r["filename"]).first()
            if existing:
                ...
                Media.objects.create(filename=r["filename"], **defaults)
                created = True
